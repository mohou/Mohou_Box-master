...
#         self.correct_baudrate = None
        ...
        self.sd_print_flag = False
        self.sd_file_name = None
        self.sd_file_size = None
        self.sd_printed_bytes = None
        self.sd_status_request_timeout = 3
...
#                 return False
    ...
    def define_regexps(self):
        ...
        self.sd_file_opened = re.compile("File opened:\s*(.*?)\s+Size:\s*([0-9]*)")
        self.sd_printing_byte = re.compile("([0-9]*)/([0-9]*)")
    ...
    def parse_printer_answers(self, line):
        if line is None:  # None means error reading from printer, but "" is ok
            ...
            match = self.sd_file_opened.search(line)
            if match:
                self.sd_print_flag = True
                self.sd_printing_flag = True
                self.sd_file_name = match.group(1)
                self.sd_file_size = int(match.group(2))
                self.printer_state = 7
                self.print_progress = 0.0
                self.start_time = time.time()
        elif 'Done printing file' in line:
            self.sd_printing_flag = False
            self.printer_state = 9
            self.print_progress = 1.0
            self.pushMessage2Client()
        elif 'SD printing byte' in line:
            match = self.sd_printing_byte.search(line)
            if match:
                self.sd_printed_bytes = int(match.group(1))
                self.printer_state = 7
        elif line[0] == "T" or line[1] == "T":
            self.parse_waiting_temperature_updates(line)
            self.operational_flag = True
            self.in_heating = True
        elif self.sd_printing_flag:
            if time.time() > self.sd_status_request_timeout and not self.in_heating:
                self.send_now("M27")
                self.sd_status_request_timeout = time.time() + 1
...
#         elif self.print_thread:
    ...
    def read_state(self):
        if self.sd_print_flag:
            if self.sd_file_size and self.sd_printed_bytes:
                self.print_progress = round(1.0 * self.sd_printed_bytes / self.sd_file_size, 2)
            if self.printer_state == 7:
                if self.start_time is not None:
                    print_time = time.time() - self.start_time
                if print_time > 0 and self.print_progress > 0 and self.print_progress < 1:
                    self.print_time_escape = self.getFormattedTimeDelta(datetime.timedelta(seconds=print_time))
                    time_left  = print_time / self.print_progress - print_time
